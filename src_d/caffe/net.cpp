...
void Net<Dtype>::layrub(const NetParameter &param){
	...
	string previous_layer_type[] = {"SoftmaxWithLoss", "InnerProduct", "Pooling", "Convolution", "Concat", "Eltwise"};//previos layer
...
void Net<Dtype>::to_cpu(const NetParameter &param){
	...
	for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
		...
		const LayerParameter& layer_param = param.layer(layer_id);
	    if (layer_param.type() == "ReLU" || layer_param.type() == "Dropout") {
	    	relu_layer_ids_.insert(layer_id);
	    } else if(layer_param.type() != "SoftmaxWithLoss" && layer_param.type() != "Split") {
...
//			}
			...
			for(int bottom_id = 0; bottom_id < bottom_vecs_[layer_id- split_layer_num].size(); ++bottom_id){
				bottom_vecs_[layer_id - split_layer_num][bottom_id]->decreaseRef();
				LOG(INFO)<<"\tlayer_id="<<layer_id<<", split_layer_num="<<split_layer_num;
				LOG(INFO)<<"\tdecrease, layer " << layer_names_[layer_id- split_layer_num]<< ", bottom blob: "
					<<blob_names_[bottom_id_vecs_[layer_id- split_layer_num][bottom_id]]
					<<", ref = "<<*(bottom_vecs_[layer_id- split_layer_num][bottom_id]->Ref());
			}
			for(int bottom_id = 0; bottom_id < bottom_vecs_[layer_id - split_layer_num - 1 - inplace_layer_num ].size(); ++bottom_id){
				if(layer_id > 1){
					bottom_vecs_[layer_id - split_layer_num - 1 - inplace_layer_num][bottom_id]->decreaseRef();
					LOG(INFO)<<"\tlayer_id="<<layer_id<<", split_layer_num="<<split_layer_num<<"inplace_layer_num, "<<inplace_layer_num;
					LOG(INFO)<<"\tdecrease, layer " << layer_names_[layer_id - split_layer_num - 1 - inplace_layer_num]<< ", bottom blob: "
						<<blob_names_[bottom_id_vecs_[layer_id - split_layer_num - 1 - inplace_layer_num][bottom_id]]
						<<", ref = "<<*(bottom_vecs_[layer_id - split_layer_num - 1 - inplace_layer_num][bottom_id]->Ref());
				}
			}
			split_layer_num = 0;
			inplace_layer_num = 0;
...
//				<<", bottom data "<< blob_names_[bottom_id_vecs_[i][0]];
			bottom_vecs_[i][0]->TransferToCPU(stream);
		}
		loss += layer_loss;
	}
	return loss;
...
//			", bottom data "<< blob_names_[bottom_id_vecs_[i-2][0]];;
			bottom_vecs_[i - 2][0]->TransferToGPU(stream);
		}
	}
}
